...
1. Can SVM be used for unsupervised clustering or data dimension reduction? Why?
   ...
   可以用于数据归约。为了控制支持向量的数量，考虑了稀疏性约束的核SVM优化。

1. What are the strengths of SVMs; when do they perform well?

   当类别之间有明确的间隔时，SVM的效果相对较好。SVM在高维空间中表现出更好的性能。SVM在维数大于样本数的情况下是有效的。SVM的存储效率相对较高。

2. What are the weaknesses of SVMs; when do they perform poorly?

   不适用于大数据集。当数据集有更多的噪声时，即目标类别重叠时，SVM的表现不是很好。如果每个数据点的特征数量超过训练数据样本的数量，SVM将表现不佳。由于支持向量分类器的工作原理是将数据点放置在分类超平面的上方和下方，因此分类没有概率解释。

3. What makes SVMs a good candidate for the classification / regression problem, if you have enough knowledge about the data?

   数据集的类别之间有明显的间隔，维度很高。数据集不是很大，噪声也很少。
