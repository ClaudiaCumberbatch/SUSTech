...
class Conv2d(Layer):
    ...
    def backward(self, da):
        ...
        db = da.sum((0, 2, 3)) / batch_size # 损失函数对于卷积层偏置的梯度
        ...
        for i in range(h_out):
            ...
            for j in range(w_out):
                ...
                # complete your code here
                # Compute the gradient of the loss with respect to the input of the convolution operation.
                # 通过将 da 和 w 在对应的窗口上的元素相乘然后相加
...
class MaxPool2d(Layer):
    ...
    def backward(self, da):
        ...
        for i in range(h_out):
            ...
            for j in range(w_out):
                w_start = j * self.stride
                idx = self.max_indices[:, :, i, j].reshape(-1)
                rows, cols = np.unravel_index(idx, self.kernel_size)
                rows += h_start
                cols += w_start
                rows = rows.reshape(batch_size, c_out)
                cols = cols.reshape(batch_size, c_out)
                b_idx, c_idx = np.indices((batch_size, c_out))
                out[b_idx, c_idx, rows, cols] = da[:, :, i, j]
        return out
...
class FullyConnected(Layer):
    ...
    def backward(self, da):
        ...
        self.dw = np.dot(da.T, self.x) / batch_size
        self.db = np.sum(da, axis=0) / batch_size
        out = np.dot(da, self.w)
