...
def smtp():
    ...
    # conn = SMTP('10.32.40.94', int(fdns_query(SMTP_SERVER, 'P')))
...
def pop():
    ...
    # conn = POP3('10.32.40.94', int(fdns_query(POP_SERVER, 'P')))
    ...
    while True:
        try:
            ...
            if cmd == 'stat':
                ...
                print(conn.uidl(int(cmd[5:])))
            elif cmd.startswith('top '):
                index = int(cmd.split(' ')[1])
                lines = int(cmd.split(' ')[2])
                print(conn.top(index, lines))
            else:
