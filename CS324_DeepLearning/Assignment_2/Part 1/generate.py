...
# 保存生成的数据集
np.savez('circle_dataset.npz', X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)

# 可视化生成的数据集
plt.figure(figsize=(8, 6))
plt.scatter(X[y == 0][:, 0], X[y == 0][:, 1], color='blue', label='Class 0')
plt.scatter(X[y == 1][:, 0], X[y == 1][:, 1], color='red', label='Class 1')
plt.title('Generated Circle Dataset')
plt.xlabel('X1')
plt.ylabel('X2')
plt.legend()
# plt.show()
plt.savefig(f'images/circle.pdf')