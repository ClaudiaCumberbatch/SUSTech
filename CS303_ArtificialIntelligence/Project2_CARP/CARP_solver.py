...
def closest_jobs(demand_list, cur_pos, cur_cap, reverse_flag=False, value_flag=False):
    ...
    dis_list = []
    for e in cal_list:
        if e[0].demand > cur_cap: continue
        else:
            res_list.append(e)
            dis_list.append(e[1])
    if len(dis_list) == 0: return []
    index = bisect.bisect(dis_list, dis_list[0])
    return res_list[0:index]
...
# 6. 前一半最远，后一半最近
# 在六种里面选择最优返回
def complex_path_scanning(original_job_list):
    local_demand_list = copy.deepcopy(original_job_list)
    # -------------------随机一个最近-------------------
    route_list = []
    while len(local_demand_list) > 0:  # 重开, start at depot
...
def SA(startNode):
    ...
    while (cur_time <= time_limit - 1):
        ...
        if val < optimal_cost:
            ...
            optimal_cost = val
        val_x = cal_cost(x)
        if val_x > val or (T != 0 and math.exp(val_x - val) / T > random.random()):
            x = y
        cnt += 1
        t2 = time.time()
        cur_time = t2 - t1
        rem_time *= 0.99
    return optimal_route, optimal_cost


def merge_split(route_list):
    local_route_list = copy.deepcopy(route_list)
    if random.random() > SPLIT_PROB:
        num = random.randint(int((len(route_list) - 1) / 2), len(route_list) - 1)
    else:
        num = random.randint(1, len(route_list) - 1)
    # if len(route_list)>1:
    #     num = random.randint(2, len(route_list) - 1)  # 选几个拆开
    # else:
    #     num = random.randint(1, len(route_list) - 1)
    ...
    if SCAN_METHOD==0: local_route_list += complex_path_scanning(splitted_job_list)
    else: local_route_list += path_scanning(splitted_job_list)
...
METHOD_PROB = 0.3
SPLIT_PROB = 1
SCAN_METHOD = 0 # 小范围用complex
if num_non_edge > num_required_edge*0.1:
    METHOD_PROB = 0.8
    SPLIT_PROB = 0.3
    SCAN_METHOD = 1 # 大范围用simple
...
# --------------path scanning-------------
start_node = complex_path_scanning(demand_list)
optimal_route, optimal_cost = SA(start_node)
